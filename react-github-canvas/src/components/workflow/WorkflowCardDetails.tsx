...
interface WorkflowCardDetailsProps {
  wf: any;
  onOpenWorkflow?: (workflow: any) => void;
}

export function WorkflowCardDetails({ wf, onOpenWorkflow }: WorkflowCardDetailsProps) {
        ...
        onClick={async () => {
          if (!details && wf.id) {
            try {
              ...
              setDetails({ error: 'Failed to fetch workflow', wf });
            }
          }
          setShowDetails(s => !s);
        }}
